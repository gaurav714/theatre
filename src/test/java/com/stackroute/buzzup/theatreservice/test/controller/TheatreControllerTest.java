...
public class TheatreControllerTest {
	...
	public void saveTheatreTest() throws Exception
	{
		...
		mockMvc.perform(post("/api/v1/theatre").contentType(MediaType.APPLICATION_JSON)
	...
	{
		assertNotNull("hi");
		when(theatreServiceImpl.updateTheatre(theatre.getTheaterId())).thenReturn(theatre);
		mockMvc.perform(post("/api/v1/theatre").contentType(MediaType.APPLICATION_JSON)
		.content(asJsonString(theatre))).andExpect(status().isOk()).andDo(print());
